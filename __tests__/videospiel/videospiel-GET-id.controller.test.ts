...
import { type VideospielModel } from '../../src/videospiel/rest/videospiel-get.controller.js';
...
describe('GET /:id', () => {
    ...
    each(idVorhanden).test(
        ...
        async (id: string) => {
            ...
            // when
            const response: AxiosResponse<VideospielModel> = await client.get(
                url,
            );
    ...
    each(idNichtVorhanden).test(
        'Kein Videospiel zu nicht-vorhandener ID %s',
        async (id: string) => {
            // given
            const url = `/${id}`;
    ...
    each(idVorhandenETag).test(
        'Videospiel zu vorhandener ID %s mit ETag %s',
