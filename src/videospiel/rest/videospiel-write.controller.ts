...
export type VideospielUpdateDTO = Omit<
    Videospiel,
    'aktualisiert' | 'erzeugt' | 'id' | 'schlagwoerter' | 'version'
>;
...
@ApiBearerAuth()
export class VideospielWriteController {
    readonly #service: VideospielWriteService;

    readonly #logger = getLogger(VideospielWriteController.name);
    ...
    /**
     * Ein neues Videospiel wird asynchron angelegt. Das neu anzulegende Videospiel ist als
    ...
    @ApiOperation({ summary: 'Ein neues Videospiel anlegen' })
    @ApiCreatedResponse({ description: 'Erfolgreich neu angelegt' })
    @ApiBadRequestResponse({ description: 'Fehlerhafte Videospieldaten' })
    async create(
        @Body() videospielDTO: VideospielDTO,
        @Req() req: Request,
        @Res() res: Response,
    ): Promise<Response> {
    ...
    /**
     * Ein vorhandenes Videospiel wird asynchron aktualisiert.
     *
     * Im Request-Objekt von Express muss die ID des zu aktualisierenden Videospiels
     * als Pfad-Parameter enthalten sein. Außerdem muss im Rumpf das zu
     * aktualisierende Videospiels als JSON-Datensatz enthalten sein. Damit die
    ...
    @Roles('admin', 'mitarbeiter')
    @ApiOperation({
        summary: 'Ein vorhandenes Videospiel aktualisieren',
        tags: ['Aktualisieren'],
    })
    @ApiHeader({
        name: 'If-Match',
    ...
    @ApiBadRequestResponse({ description: 'Fehlerhafte Videospiedaten' })
    @ApiPreconditionFailedResponse({
    ...
    async update(
        @Body() videospielDTO: VideospielUpdateDTO,
        @Param('id') id: string,
        @Headers('If-Match') version: string | undefined,
        @Res() res: Response,
    ): Promise<Response> {
        this.#logger.debug(
            'update: id=%s, videospielDTO=%o, version=%s',
            id,
            videospielDTO,
            version,
        );
        ...
        const result = await this.#service.update(
            id,
            this.#updateDtoToVideospiel(videospielDTO),
            version,
        );
    ...
    /**
     * Ein Videospiel wird anhand seiner ID-gelöscht, die als Pfad-Parameter angegeben
    ...
    @Roles('admin')
    @ApiOperation({
        summary: 'Videospiel mit der ID löschen',
        tags: ['Loeschen'],
    })
    @ApiHeader({
        name: 'Authorization',
    ...
    @ApiNoContentResponse({
        description: 'Das Videospiel wurde gelöscht oder war nicht vorhanden',
    })
    async delete(
        @Param('id') id: string,
        @Res() res: Response,
    ...
    #dtoToVideospiel(videospielDTO: VideospielDTO): Videospiel {
        ...
        // Rueckwaertsverweis von Schlagwort zu Videospiel
    ...
    #updateDtoToVideospiel(videospielDTO: VideospielUpdateDTO): Videospiel {
        const videospiel: Videospiel = {
            id: undefined,
            version: undefined,
            titel: videospielDTO.titel,
    ...
    #handleUpdateError(err: UpdateError, res: Response): Response {
        switch (err.type) {
            ...
            case 'VideospielNotExists': {
                const { id } = err;
                const msg = `Es gibt kein Videospiel mit der ID "${id}".`;
                this.#logger.debug('#handleUpdateError: msg=%s', msg);
                return res
